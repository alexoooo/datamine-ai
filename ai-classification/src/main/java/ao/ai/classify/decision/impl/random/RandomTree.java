...
import ao.ai.classify.decision.impl.input.processed.attribute.Continuous;
...
public class RandomTree implements Serializable
{
    ...
    public static RandomTree nextRandom(LocalLearningSet ls)
    {
        ...
        int length = Math.min(16,
                (int)(attributeList.size() * 0.55 + 0.99));
        assert length >= 1;
...
//    private int       sampleSize;
    ...
    private void splitNext(Attribute attributes[], int atIndex)
    {
        ...
        if (! split.isSingleUse() &&
                ! ((Continuous) split).isWellInformed())
        {
